export const Data = {
  profileHead: {
    id: 0,
    sub: "關於",
    title: "About",
    p: `我叫呂季韋，是一個熱愛挑戰和享受團隊工作的前端工程師。
        第一份工作任職於一間客製化網站設計公司，主要負責網站的開發和切版，根據客戶需求做出符合的功能和網站動態效果。而在職期間除了精進技術實力，也包括溝通能力、團隊合作、提前預防問題發生、評估開發時程等，發覺每個行業注重的地方和風格調性都不盡相同，未來期望往程式開發深耕，做出更好的產品。`,
    url: "../",
  },
  experienceHead: {
    id: 0,
    sub: "工作經歷",
    title: "Experience",
    p: "",
    url: "../",
  },
  projectHead: {
    id: 0,
    sub: "專案成就",
    title: "Portfolio",
    p: "任職期間做的專案",
    url: "../",
  },
  projectItem: {
    project: {
      data: [
        {
          id: 0,
          title: "好侶形象網站",
          time: "2020/04~2020/05",
          url: "https://www.perfectcompanion.com.tw/tw",
          img: require("../images/project01.jpg"),
          text: "資料待補充",
        },
        {
          id: 1,
          title: "潤泰創新",
          time: "2021/02~2021/06",
          url: "https://www.rt-develop.com.tw/tw",
          img: require("../images/project02.jpg"),
          text: "資料待補充",
        },
        {
          id: 2,
          title: "金豐盛Kingrich",
          time: "2021.06-2021-08",
          url: "https://www.kingrichfoods.com/tw",
          img: require("../images/project03.jpg"),
          text: "資料待補充",
        },
        {
          id: 3,
          title: "法藍瓷購物網站",
          time: "2021/12~2022/03",
          url: "https://www.franzcollection.com.tw/tw",
          img: require("../images/project04.jpg"),
          text: "資料待補充",
        },
        {
          id: 4,
          title: "台積心築藝術季2022",
          time: "2022/04~2022/04",
          url: "https://www.tsmc-foundation.org/art-festival/2022/",
          img: require("../images/project05.jpg"),
          text: "資料待補充",
        },
        {
          id: 5,
          title: "增你強股份有限公司",
          time: "",
          url: "https://www.zenitron.com.tw/tw",
          img: require("../images/project06.jpg"),
          text: "資料待補充",
        },
        {
          id: 6,
          title: "台中港酒店",
          time: "2023/01~2023/03",
          url: "https://tchhotel.com/",
          img: require("../images/project07.jpg"),
          text: "資料待補充",
        },
      ],
    },

    // abc: {
    //   data: [
    //     {
    //       id: 0,
    //       title: "abc",
    //       time: "2021.06-2021-08",
    //       url: "",
    //       img: require("../images/1.png"),
    //       text: "aaa",
    //     },
    //     {
    //       id: 1,
    //       title: "abc",
    //       time: "2021.06-2021-08",
    //       url: "",
    //       img: require("../images/2.png"),
    //       text: "123",
    //     },
    //     {
    //       id: 2,
    //       title: "abc",
    //       time: "2021.06-2021-08",
    //       url: "",
    //       img: require("../images/3.png"),
    //       text: "123",
    //     },
    //   ],
    // },
  },
  contactHead: {
    id: 0,
    sub: "聯絡資訊",
    title: "Contact",
    p: "請寄信到 ericlu0814@gmail.com",
    url: "../",
  },
};
